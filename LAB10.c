 ...
 * VARIABLES 
 ------------------------------------------------------------------------------*/
char mensaje[12] = {'\r','1','.','L','e','e','r',' ','p','o','t','\r'};
char enviar[16] = {'2','.','E','n','v','i','a','r',' ','A','S','C','I','I','\r',' '};
char error[7] = {'\r','e','r','r','o','r','\r'};
int indice; 
    ...
    if(PIR1bits.ADIF){              // Fue interrupci n del ADC??
        if(ADCON0bits.CHS == 0){    // Verificamos sea AN0 el canal seleccionado
            ...
            for (cont = 0; cont < 3; cont++){ // CICLO FOR PARA OBTENER LOS VALORES 
             if (cont == 0)
                u = convertir(valor, cont); //SE OBTIENE EL VALOR DE UNIDAD  
             else if (cont == 1) 
                d = convertir(valor, cont);// SE OBTIENE EL VALOR DE LA DECENA
             else if (cont == 2) 
               c = convertir(valor, cont);
            } 
            numero[0] =c;
            numero[1] =d;
            numero[2] =u;
